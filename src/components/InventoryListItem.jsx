...
const InventoryListItem = (props) => {
   ...
   */
  /* istanbul ignore next */
  const ButtonType = ({ id, item, itemInCart, price }) => {
    const label = itemInCart ? "Remove" : "Add to cart";
    const onClick = itemInCart ? () => {
      removeFromCart(id);
      window.dataLayer.push({
        ...
        clickedButton: label + ':' + item + ':' + price
      });
    } : () => {
      addToCart(id);
      window.dataLayer.push({
    ...
    // const onClick = itemInCart ? () => removeFromCart(id) : () => { console.log(price) }
